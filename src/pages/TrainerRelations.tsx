...
import { ChevronLeft, ChevronRight, Eye, Link2Off, Search, UserCheck, Users } from 'lucide-react'
...
export function TrainerRelationsPage() {
  ...
  const formatClientCardTitle = (relation: TrainerClientRelation): string => relation.client_display_name?.trim() || relation.client_login?.trim() || 'Клиент'
  const formatClientCardSubtitle = (relation: TrainerClientRelation): string | null => {
      ...
      <Card className="overflow-hidden border-primary/30">
        ...
        <CardContent className="grid gap-3">
          <div className="rounded-xl border border-border/70 bg-secondary/30 p-3">
            <span className="mb-3 block text-sm">Статус тренера в поиске: {trainerVisibilityLabel}</span>
            <Button
              size="sm"
              onClick={() => {

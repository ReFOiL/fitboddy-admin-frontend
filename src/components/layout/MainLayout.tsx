...
export function MainLayout() {
  ...
  const mobileGridClass = isTrainer ? 'grid-cols-6' : isClient ? 'grid-cols-4' : 'grid-cols-3'
  ...
  return (
    <div className="min-h-dvh bg-background text-foreground">
      ...
      <main className="mx-auto w-full max-w-6xl px-4 py-4 pb-28 md:px-6 md:py-6 md:pb-6">
        <Outlet />
      </main>
      <nav className="fixed inset-x-0 bottom-0 z-30 border-t border-border bg-background/95 pb-[env(safe-area-inset-bottom)] backdrop-blur md:hidden">
        <div
          className={`mx-auto grid h-16 w-full max-w-6xl items-center px-2 text-xs sm:px-4 sm:text-sm ${mobileGridClass}`}
        >
          <NavLink className={navItemClass} to="/home">
